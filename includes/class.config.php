 ...
 * @author      VibeThemes
 * @category    Admin
 * @package     PLUGIN_DOMAIN/Includes
 * @version     1.0
 */
...
class Wplms_Custom_Certificates_Codes_Update{
	...
	public static function init(){
        ...
        if ( is_null( self::$instance ) )
            self::$instance = new Wplms_Custom_Certificates_Codes_Update();
	...
	function check_addon_license_status($class,$addon){
		...
		if($addon['license_key'] != 'wplms_custom_certificate_codes_license_key')
			return $class;
	...
	function sanitize_license( $new ) {
		$old = get_option( 'wplms_custom_certificate_codes_license_key' );
		if( $old && $old != $new ) {
	...
	*************************************/
	...
	function wplms_pdf_certificates_activate_license($message) {
		...
		if( empty($_POST['license_key']) || empty($_POST['wplms_custom_certificate_codes_license_key']) || $_POST['wplms_custom_certificate_codes_license_key'] != 'Activate') 
			return;
		...
		if ( is_wp_error( $response ) ){
			return __('Unable to contact server','PLUGIN_DOMAIN');
		}
		...
		if(!empty($license)){
			...
			update_option('wplms_custom_certificate_codes_license_key',$license);
		}
		return $message;
	}
	...
	***********************************************/
	...
	function wplms_pdf_certificates_deactivate_license() {
		...
		if( empty($_POST['license_key']) || empty($_POST['wplms_custom_certificate_codes_license_key']) || $_POST['wplms_custom_certificate_codes_license_key'] != 'Deactivate') 
			return;
		...
		// run a quick security check
	 	if( ! check_admin_referer( 'PLUGIN_DOMAIN', 'PLUGIN_DOMAIN' ) )
			return; // get out if we didn't click the Activate button

		// retrieve the license from the database
		$license = trim( get_option( 'wplms_custom_certificate_codes_license_key' ) );
		...
		if( $license_data->license == 'deactivated' ){
			...
			delete_option( 'wplms_custom_certificate_codes_license_key' );
		}
...
Wplms_Custom_Certificates_Codes_Update::init();